...
docker exec elasticsearch curl -s -u elastic:${ELASTIC_PASSWORD} -X POST "http://localhost:9200/_security/user/temp_init_user" -H "Content-Type: application/json" -d '{"password":"temp123","roles":["superuser"]}' >/dev/null 2>&1
echo "Security index initialized."

echo "Step 4: Deleting old token (if exists)..."
docker exec elasticsearch /usr/share/elasticsearch/bin/elasticsearch-service-tokens delete elastic/kibana kibana-token 2>/dev/null || true

echo ""
echo "Step 5: Creating new Kibana service account token..."
TOKEN_OUTPUT=$(docker exec elasticsearch /usr/share/elasticsearch/bin/elasticsearch-service-tokens create elastic/kibana kibana-token)
